...
type textifyTraverseContext struct {
	...
	lineLength      int
	isPre           bool
	isVirtualBQ     bool // virtual blockquote
}
...
func (ctx *textifyTraverseContext) handleElement(node *html.Node) error {
	...
	case atom.Blockquote:
		if ctx.buf.Len() == 0 && !ctx.isVirtualBQ { // do not apply blockquote if full html is blockquote
			return ctx.traverseChildren(node)
		}

		ctx.blockquoteLevel++
		ctx.prefix = strings.Repeat(">", ctx.blockquoteLevel) + " "
		// following lines are disabled, otherwise it outputs 2x empty '>' lines
		//if err := ctx.emit("\n"); err != nil {
		//	return err
		//}
		...
		// to remove the last "> " (or multiple on levels) added we would have to make some magic with the ctx.buf
		return ctx.emit("\n\n")

	case atom.Div:
		// hack <div class="quote_container"> as blockquote
		for _, attr := range node.Attr {
			if attr.Key == "class" && attr.Val == "quote_container" {
				node.DataAtom = atom.Blockquote
				ctx.isVirtualBQ = true
				err := ctx.handleElement(node)
				ctx.isVirtualBQ = false
				return err
			}
		}
	...
	case atom.Noscript:
		// Because of bug https://github.com/golang/go/issues/16318 we have to remove the whole content in <noscript> tags.
		// The bug is that treating it as div above would show the whole content between the noscript tags as text instead of traversing it's HTML elements.
		return nil
